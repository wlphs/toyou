...
function openSnippet(msg, $delay) {
    var ran = Math.floor(1e7 * Math.random()), _delay = $delay || 2e3;
    ...
    $(".snippet_cont", snippet).animate({
        top: 0
    }, 300).delay(_delay).fadeOut(300, function() {
        snippet.remove();
    });
}
...
$(function() {
    $("#footer .js-open").click(function() {
        ...
        $("#footer").length && (_toppos = _st - $("#footer").offset().top + _h + 20), 100 < _st ? $(".top_scroll").stop().fadeIn() : $(".top_scroll").stop().fadeOut(), 
